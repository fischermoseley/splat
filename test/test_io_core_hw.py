...
class IOCoreLoopbackTest(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += [
            self.s.io_core.probe0.eq(self.s.io_core.probe4),
            self.s.io_core.probe1.eq(self.s.io_core.probe5),
            self.s.io_core.probe2.eq(self.s.io_core.probe6),
            self.s.io_core.probe3.eq(self.s.io_core.probe7),
    ...
    def verify_output_probe_initial_values(self):
        ...
        for name, attrs in outputs.items():
            actual = self.s.io_core.get_probe(name)
    ...
    def verify_probes_update(self):
        ...
        for input, output in zip(inputs, outputs):
            ...
            self.s.io_core.set_probe(output, value)
            readback = self.s.io_core.get_probe(input)
