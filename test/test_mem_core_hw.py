...
class MemoryCoreLoopbackTest(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += [
            self.s.mem_core.user_addr.eq(self.s.io_core.addr),
            self.s.mem_core.user_data.eq(self.s.io_core.data),
            self.s.mem_core.user_we.eq(self.s.io_core.we),
    ...
    def write_user_side(self, addr, data):
        self.s.io_core.set_probe("we", 0)
        self.s.io_core.set_probe("addr", addr)
        self.s.io_core.set_probe("data", data)
        self.s.io_core.set_probe("we", 1)
        self.s.io_core.set_probe("we", 0)

    def verify_register(self, addr, expected_data):
        data = self.s.mem_core.read_from_user_addr(addr)
